<template>
  ...
  <div v-if="!isTodoEmpty">
    ...
    <ModalShow :show="!!selectedItem" @close="selectedItem = null">
      ...
      <template v-if="selectedItem?.has_description" #description>{{ selectedItem?.description }}</template>
    </ModalShow>
    ...
    <div class="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-1">
      ...
      >
        <div class="flex justify-between gap-4">
          ...
          <p class="text-sm font-extralight text-center shrink-0 rounded-full bg-blue-400 text-white w-5 h-5">{{ item.id }}</p>
        </div>
        <div class="flex mx-2 justify-center gap-1">
          <button
            ...
            class="p-2 rounded-md bg-blue-600 text-white hover:bg-blue-700 transition"
          >
            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M120-120v-320h80v184l504-504H520v-80h320v320h-80v-184L256-200h184v80H120Z"/></svg>
